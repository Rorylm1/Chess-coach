...
Three top-level tabs in the long-term vision: **Play** (vs the bot or a human) · **Openings** (the M5 journeys) · **Coach** (post-game review / grounded teaching). They land across milestones — **M6** = Play + Openings tabs · **M7** = online play · **M8** = the Coach tab · **M9** = release. Within Play, a `Bot · Multiplayer` mode switch drives one generalized loop; **coach + analysis are bot-mode only** (a hot-seat game is just play — no engine assist). **M6 shipped** — Play + Openings tabs, a per-game **randomized 2D board** (the "Dealer's roll" randomizer: seeded + OKLCH, unique every game yet always legible, classic board the default; `lib/board/` + `components/BoardRandomizer/`; built **exploration-first** per `distinctive_design.md` via `research/designs/board-randomizer.html`, engineering spine in `research/randomizer-color-system.md`), and local **hot-seat** 2-player. **The per-game randomizer has since evolved into the _Generative Table_** (Play tab only): clicking *Deal a table* makes a live Claude call (`POST /api/table`) that **invents a complete, fresh design every game** — surfaces, typography (any Google font, loaded on the fly), accents, board palette _and_ recolorable SVG pieces — rendered safely from a **validated spec** (`lib/table/`, mapped onto the app's existing `:root` tokens so the whole Play screen reskins with no CSS rewrites) and guaranteed legible by a deterministic backstop. It is **ephemeral** (nothing persisted — the classic Deep-Space look is always the default) and subsumes the old board-randomizer control into one "Deal a table". Built **exploration-first** (`research/designs/generative-table.html` + `table-server.mjs`; the high-entropy prompt approach in `research/randomize-prompt-research.md`). Current focus is **M7** — a **shareable live link** (the one deliberate step beyond local-first). The original weakness-tracking arc (old M6 "Remember") is **deferred** to `ideas.md`.
...
**Licensing guardrail:** keep our code permissive. Stockfish WASM stays an isolated static asset; **never** add GPL JS libraries (`chessground`, `chessops`). Generative-table piece SVGs are **permissive only** — chessnut (Apache-2.0) ships today; never use GPL/non-commercial sets (cburnett = GPL, maestro/fresca/staunty = CC-BY-NC-SA). Permissive families for the library: maurimo (MIT), RhosGFX (CC0), kiwen-suwi (CC-BY), cburnett via the Wikimedia BSD grant.
